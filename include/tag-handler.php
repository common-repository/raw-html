	...
	foreach ($tags as $tag_pair){
		...
		while( preg_match($start_regex, $text, $matches, PREG_OFFSET_CAPTURE, $offset) === 1 ) {
			...
			if ( $is_excerpt_stripping_enabled
				...
				//Note: This plugin doesn't directly hook into "get_the_excerpt" 
				//or "the_excerpt", only "the_content". wp_trim_excerpt() calls
				//"the_content" when it auto-generates an excerpt.
				$replacement = '';
			} else {
